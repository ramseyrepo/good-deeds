...
- 2026-07-16 — build — public volunteer profiles. Added User.username (unique), User.bio, User.profilePublic (default false, OPT-IN). Public page /u/[username] (added to proxy publicRoutes as "/u/:path*"; robots index) shows name, bio, member-since, impact stats, activity breakdown, attended deeds. Edit page /app/profile with visibility toggle; updateProfile action validates username format + uniqueness. Public participants/organizers on event pages link to their profile. getCurrentUser mints a unique username on first login.
- 2026-07-16 — carte — registered public page. route_id "volunteer-profile" (path /u/[username]), robots index,follow, sitemap weekly/0.6. Note: page also emits per-user dynamic metadata via generateMetadata.
- 2026-07-16 — drift/db — manual SQL migration. Live User table had 2 rows, so `prisma db push` couldn't add required unique `username`. Added username/bio/profilePublic via spark_run_sql and backfilled usernames from email (e.g. ramseyross-56a7) instead of --force-reset (no data loss). DB now in sync with schema.
- 2026-07-16 — design — rebrand to "Ulu". Renamed platform Good Deeds → Ulu across UI/metadata/README. New minimalist logo (src/components/gd/ulu-mark.tsx): breadfruit body + single growth leaf + skin dots, single-color via currentColor. Favicon updated (white mark on forest square) — login page logo auto-updates from deployed /favicon.svg. NOTE (needs Souped MCP, currently disconnected): update Carte "volunteer-profile" title (still says "Good Deeds"), and optionally rename Vercel subdomain/URL from good-deeds.getsouped.app.
